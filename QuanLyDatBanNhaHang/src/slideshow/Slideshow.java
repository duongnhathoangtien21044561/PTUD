...
public class Slideshow extends JLayeredPane{
                ...
                if (!animator.isRunning()) {
                    if (pageClick != currentIndex) {
                        ...
                        if (next) {
                            ...
                            componentOut = panel.getComponent(checkPrevious(currentIndex));
                            currentIndex = getPrevious(pageClick + 1);
                            componentShow = panel.getComponent(currentIndex);
                            animator.start();
                        }
                    }
    ...
    public void previous(){
        if (!animator.isRunning()) {
            timer.restart();
            next = false;
            currentIndex = getPrevious(currentIndex);
            componentShow = panel.getComponent(currentIndex);
            componentOut = panel.getComponent(checkPrevious(currentIndex + 1));
            animator.start();
        }
    }
    ...
    private int checkPrevious(int index){
